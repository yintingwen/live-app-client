<template>
	<div class="show-box"  :class="{disabled: disabled, active: isShowList, 'no-border-radius': !borderRadius}" :style="style_Container">
		<!-- 输入框，仅在可输入模式下使用 -->
		<input  
		...
		:style="'top:' + listTop__ + 'px;'" v-show="isShowList">
			...
			<scroll-view 
				class="list" :class="{'no-border-radius': !borderRadius}"
	...
	export default {
		...
		data() {
			return {
				...
				selectText: this.$t('mine.fundsTransfer.fund'),    // 已经选择的内容
  ...
  .no-border-radius{
    ...
    padding: 20rpx 50rpx 20rpx 20rpx;
		...
		.right-arrow{
			...
			position: absolute; font-size: 1em; right: 16rpx; display: flex; 
			top: 0;
		...
		.list-container{
			...
			box-sizing: border-box; z-index: 100; color: #333; text-align: center;
