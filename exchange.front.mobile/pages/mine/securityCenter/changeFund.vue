<template>
  <view class="forget-page">
    <login-header :back="true">
      <block slot="title">{{$t('mine.securityCenter.changeFund')}}</block>
    </login-header>
      ...
      <view class="c-form">
        ...
        <view class="c-form-item">
          ...
          <input
            ...
            v-model="password"
            password
            :placeholder="$t('login.placehold.new_fundPassword')"
            class="c-input"
            placeholder-class="c-input-placehold"
          />
        </view>
...
export default {
  ...
  methods: {
      ...
      const rules = [
        {
          ...
          errorMsg: this.isMobile
            ? 'login.error.inputMobile'
            : 'login.error.inputEmail'
        }
      ]
    ...
    checkForm () {
      const rules = [
        ...
        },
        {
          name: 're_password',
          checkType: 'same',
          checkRule: this.forgetForm.pay_password,
    ...
    async forget () {
      this.showFullLoading()
      try {
        await this.api.user.forgetFund(this.forgetForm)
