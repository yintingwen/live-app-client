<template>
  <view class="addAlipay-page">
    <page-layout>
      <view class="form">
        <c-inp class="mb-10" :title="$t('mine.authentication.twoLever.alipay.name.title')" :placeholder="$t('mine.authentication.twoLever.alipay.name.enter')" v-model="form.name"></c-inp>
        <c-inp class="mb-10" :title="$t('mine.authentication.twoLever.alipay.account.title')" :placeholder="$t('mine.authentication.twoLever.alipay.account.enter')" v-model="form.account"></c-inp>
...
export default {
  data() {
    return {
      pageTitle: 'mine.authentication.twoLever.alipay.add',
      ...
      const rules = [
        {
          name: 'name',
          checkType: 'notnull',
          errorMsg: 'mine.authentication.twoLever.alipay.name.check'
        },
        {
          name: 'account',
          checkType: 'notnull',
          errorMsg: 'mine.authentication.twoLever.alipay.account.check1'
        },
        {
          name: 'account',
          checkType: 'reg',
          checkRule: /^(?:\w+\.?)*\w+@(?:\w+\.)+\w+|\d{9,11}$/,
          errorMsg: 'mine.authentication.twoLever.alipay.account.check2'
        },
        {
          name: 'url',
          checkType: 'notnull',
  ...
  onLoad(e){
   ...
   if(JSON.stringify(e) !== '{}'){
      ...
      this.id = e.id
      this.pageTitle = 'mine.authentication.twoLever.alipay.edit'
